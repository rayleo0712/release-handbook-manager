...
![AI 发布治理组合套件封面](./assets/cover-zh.svg)

> 用 AI 开发系统时，最担心的就是「升级一次崩一次」。
> 这套组合包含两个配套工具：**rhm 负责把每次升级要做的事情提前整理成清单和脚本**，**rta 负责照着清单自动跑验收**，双工具闭环，照着做不出错。
...
- 不知道要按什么顺序操作
- 升级完才发现哪里不对，但已经不好回退了
- 功能验收要点来点去，每次升级都要重新测一遍

**这套组合就是把「每次升级要做的所有事情」先由 rhm 整理成清单和脚本，再由 rta 照着清单自动执行验收，双工具闭环解决升级容易崩溃的问题。**

---

## 这套组合到底帮你解决了哪些具体问题

| 升级时经常遇到的坑 | 组合里的哪个工具来填 | 怎么帮你填上 |
|---|---|---|
| 「这次到底改了哪些东西？升级前根本记不全」 | rhm | 自动帮你整理一份更新手册，改了什么一目了然 |
| 「数据库脚本散落在聊天记录里，上线时找不到正确的那一份」 | rhm | 所有脚本统一放在一个文件夹里，编号排好顺序 |
| 「菜单、权限、角色都是后台手工加的，每次升级都要重新点一遍」 | rhm | 自动生成可以直接执行的 SQL 脚本，一条命令搞定，不用手工点 |
| 「脚本跑完不知道有没有成功，只能看日志猜」 | rhm | 每条脚本都自带「成功怎么判断」的验证语句，跑完运行一下就知道过没过 |
| 「AI 经常自动乱改版本号，越改越乱」 | rhm | 版本号强制人工把关，AI 不能自己瞎改（一票否决红线） |
| 「升级时操作顺序全靠记忆，顺序错了系统就崩」 | rhm | 发布前有检查清单，发布步骤按顺序一条条写好 |
| 「升级完要给领导/客户写更新说明，又要重新整理一遍」 | rhm | 自动从已整理的材料里提炼出更新说明，不用再重写 |
| 「功能验收要点来点去，每次升级都要重新测一遍」 | rta | 配套自动化测试工具，照着 05-1 验收清单自动跑，还支持断点续跑 |
| 「测到一半会话断了/电脑休眠了，又要从头重新测」 | rta | 每条用例结果立即落盘，中断了可以从断点接着跑 |
| 「测完了 bug 修好了，不知道该不该全部重测一遍」 | rta | 支持修复 bug 后重启服务、按需恢复数据库基线、断点续跑而非全量重测 |
...
以前的升级：
> 开发临时回忆改了什么 → 到处找脚本 → 群里问有没有要改的配置 → 凭感觉执行 → 人工点点点验收 → 出问题再抢修

用了这套组合之后的升级：
> 开发过程中 rhm 边改边把要升级的内容记下来 → 升级前所有脚本、步骤、验证方法全部整理齐全 → 按清单一步一步执行 → 每一步跑完都有验证 → rta 自动读取验收清单自动跑 → 结果自动回填归档 → 结束后自动生成更新说明

**简单说：从「凭经验升级 + 人工点点验收」变成「按清单升级 + 自动化验收」的完整闭环。**
